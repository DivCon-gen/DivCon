...
if args.dataset == 'HRS':
    ...
    with open('../gt_benchmark/NSR-1K/counting.val.json', 'rb') as f:
        NSR = json.load(f)
...
for i, prompt in tqdm(enumerate(gt), total=len(gt), desc="Processing"):
    if i>=0 and prompt not in pred_layout:
        #1st stage
        messages = load_json('example1.json')
        messages.append(
        ...
        #2nd stage
        message = 'Provide box coordinates for an image with ' + obj_nums + '. ' + prompt
        messages = load_json('example2.json')
        messages.append(
        ...
        if args.dataset == 'HRS':
            with open("HRS_counting.p", "wb") as pickle_file:
                pickle.dump(pred_layout, pickle_file)
        else:
            with open("NSR1K_counting.p", "wb") as pickle_file:
                pickle.dump(pred_layout, pickle_file)
