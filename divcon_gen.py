...
from chatGPT import read_txt_hrs, load_gt, load_box, save_img, read_csv, generate_box_counting, generate_box_spatial, Pharse2idx_2, process_box_phrase, format_box, draw_box_2
import torchvision.transforms as transforms
...
def run(meta,models,info_files, p, starting_noise=None,iter_id=0, img_id=0, save=True, stage='img'):
    ...
    if stage == 'fg':
        ...
        for i, box in enumerate(all_boxes):
            ...
            try: cropped_img = Image.fromarray(cropped_img.astype(np.uint8))
            except: continue
    ...
    if save :
        ...
        start = len( os.listdir(output_folder1) )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", choices=['counting','spatial'])
    parser.add_argument("--pred_layout", type=str, default=None)
    parser.add_argument("--file_save", type=str)
    parser.add_argument("--data_type", choices=['HRS','NSR-1K'], default='HRS')
    ...
    parser.add_argument("--use_llm", action='store_true', help='choose the way to generate layout')
    ...
    if args.data_type=='HRS':
        if args.type=='counting':
            _ ,prompts = load_gt('gt_benchmark/HRS/counting_prompts.csv')
        else:
            prompts, _ = load_gt('gt_benchmark/HRS/spatial_compositions_prompts.csv')

    elif args.data_type=='NSR-1K':
        if args.type=='counting':
            prompts = read_csv('../gt_benchmark/drawbench/drawbench.csv','Counting')
            list_box = ['../gt_benchmark/gpt_generated_box_drawbench/counting.p']
        else:
            prompts = read_csv('../gt_benchmark/drawbench/drawbench.csv','Positional')
            list_box = ['../gt_benchmark/gpt_generated_box_drawbench/spatial.p']
    # box_data = load_box(list_box[0])
    if args.use_llm==False:
        box_data = load_box(args.pred_layout)
        keys = box_data.keys()
        ...
        try:
            for meta in meta_list:
                ...
                if args.use_llm == False:
                    ...
                    if args.type == 'counting': o_names, o_boxes = generate_box_counting(text)
                    else: o_names, o_boxes = generate_box_spatial(text)
                ...
                for k in range(1):
                    ...
                    l = np.array(o_boxes)
                    #fix box error
                    for pos, box in enumerate(l):
                        left, top, right, bottom = box
                        # fix box coord error
                        l[pos][0] = min([left, right])
                        l[pos][1] = min([top, bottom])
                        l[pos][2] = max([left, right])
                        l[pos][3] = max([top, bottom])
                    ...
                    fg_noise, worse_id, froze_area = run(meta, models, info_files, args, starting_noise, k,i, stage='fg')
                    ...
                    run(meta, models, info_files, args, fg_noise, k,i)
